...
export default defineEventHandler(async (event) => {
  ...
  try {
    const projectDoc = await couch.request<any>(`/facilis-projects/${projectId}`);
    
    if (!projectDoc.developers?.includes(user.name) && !projectDoc.users?.includes(user.name)) {
    ...
    throw new HTTPError('Failed to fetch project settings', { status: 500 });
  }
});
