...
export default defineEventHandler(async (event) => {
  ...
  if (!body?.projectId || !body?.messages) {
    throw new HTTPError("projectId and messages are required", { status: 400 });
  }
  ...
  if (!systemPrompt) {
    try {
      // Use useStorage if available (Nitro v3)
      ...
      // Fallback to direct FS read in dev if useStorage is missing
      if (!systemPrompt) {
        const fs = await import('fs/promises');
        const path = await import('path');
        const promptPath = path.join(process.cwd(), 'server/assets/prompts/default_system.md');
        systemPrompt = await fs.readFile(promptPath, 'utf-8');
      }
    } catch (e) {
      console.error('Failed to load default system prompt asset', e);
      // Last resort fallback
      systemPrompt = "You are facilis.dev AI, an expert web developer specializing in rapid prototyping. Goal: Help build an application.";
    }
  }
